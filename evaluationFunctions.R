...
EvaluateBioclimModel<-function(folds, covs.pres, covs.bkg.train, covs.bkg.test, threshold=NULL){
  if(is.null(threshold)){
    results<-data.frame(n.train=rep(0,folds), n.test=0, train.auc=0,
                        test.auc=0, p.value= 0, stringsAsFactors=FALSE)
    kvector <- kfold(covs.pres, folds)
    ...
    for (k in 1:folds){
      ...
      auc.train <- evaluate(pocc.train[y.train==1], pocc.train[y.train==0])@auc
      auc.test <- evaluate(pocc.test[y.test==1], pocc.test[y.test==0])@auc
      p.value <- evaluate(pocc.test[y.test==1], pocc.test[y.test==0])@pauc
      if(length(p.value)==0) p.value <- NA
      results[k, ]<-c(n.train, n.test, auc.train, auc.test, p.value)
    }
    return(results)
  } else {
    ...
    for (k in 1:folds){
      ...
      n.test <- length(which(kvector==k))
      bc.obj <- bioclim(covs.pres[kvector!=k, ])
      p.train <- predict(bc.obj, covs.pres[kvector!=k, ])
      p.test <- predict(bc.obj, covs.pres[kvector==k, ])
      a.test <- predict(bc.obj, covs.bkg.test)
      t.value <- quantile(p.train, threshold/100, na.rm=T)
      p <- p.test >= t.value
      p.area <- sum(a.test >= t.value)/length(a.test)
      sens <- sum(p)/length(p)
      tss <- sens - p.area
      p.value <- binom.test(sum(p), length(p), p = p.area, alternative = "greater", conf.level = 0.95)$p.value
      results[k, ] <- c(n.train, n.test, sens, 1-p.area, tss, p.value)
    }
    return(results)
  }
}

EvalCH <- function(occs, env.vars, buffer, proj.crs){
  results <- data.frame(sens = rep(NA,nrow(occs)), spec=NA, tss= NA, p.value = NA)
  if(nrow(occs)>1){
    for(i in 1:nrow(occs)){
      ch.map <- ChMap(occs[-i, ], env.vars, buffer = buffer, proj.crs)
      p <- na.omit(extract(ch.map, cbind(occs$lon[i], occs$lat[i])))
      if(length(p)==0){
        next()
      }
      results$sens[i] <- sum(p) / length(p)
      p.area <- cellStats(ch.map, sum) / cellStats(!is.na(ch.map), sum)
      results$spec[i] <- 1 - p.area
      results$tss[i] <- results$sens[i] + results$spec[i] - 1
      results$p.value[i] <- binom.test(sum(p), length(p), p = p.area,
                                       alternative = "greater",
...
ThresholdMXEval<-function(folds, covs.pres, covs.bkg.train, covs.bkg.test, mxnt.args, threshold){
    ...
    for (k in 1:length(kvector)){
      ...
      mx.obj <- maxent(x=rbind(covs.pres[kvector!=k, ], covs.bkg.train),
             p=c(rep(1,sum(kvector!=k)),rep(0,nrow(covs.bkg.train))),
             removeDuplicates=FALSE, args=mxnt.args)
      p.train <- predict(mx.obj, covs.pres[kvector!=k, ])
      p.test <- predict(mx.obj, covs.pres[kvector==k, ])
      a.test <- predict(mx.obj, covs.bkg.test)
